...
import ServiceBase from 'App/Service/IServiceBase'
...
export interface Link extends NovoLink {
  id: number
  visitas: number
}
/** Service que trabalha com o CRUD de objetos do tipo `Link` e se comunica com o banco.
 * @see Link */
class LinkService implements ServiceBase<Link> {
  private table: string
  constructor() {
    this.table = dbConnector.SQLTable
  }
  ...
  public async insereNoBanco(link: NovoLink): Promise<number> {
    ...
    if (isMSSQL)
      //se for mssql, dá um select do ultimo id da tabela, que será o valor inserido no comando anterior
      insertSQL += `; SELECT TOP(1) id FROM ${this.table} ORDER BY id DESC`

    return new Promise((resolve, reject) => {
      dbConnector.executeQuery(
        insertSQL,
        [link.codigo, link.url, link.nome, link.expira_em],
        (err, result) => {
          if (err) reject(err.message)
  ...
  public async pegaPorCodigo(codigo: string): Promise<Link> {
    return new Promise((resolve, reject) => {
      ...
      let where =
        dbConnector.SQLTypeInUse === SQLTypes.MySQL
  ...
  public async updateNoBanco(link: Link): Promise<void | Link> {
    return new Promise((resolve, reject) => {
      dbConnector.executeQuery(
        `UPDATE ${this.table} set codigo = ?, url = ?, nome = ?, visitas = ? where id = ?`,
        [link.codigo, link.url, link.nome, link.visitas, link.id],
        (err) => {
          if (err) reject(err.message)
          else resolve(link)
        }
  ...
  public async deletaTodosExpirados(): Promise<void> {
    return new Promise((resolve, reject) => {
      let today =
        dbConnector.SQLTypeInUse === SQLTypes.SQLServer ? 'CAST(GETDATE() AS DATE)' : 'CURRENT_DATE'

      dbConnector.executeQuery(
        `DELETE FROM ${this.table} WHERE expira_em < ${today}`,
        undefined,
        (err) => {
          if (err) reject(err.message)
          else resolve()
