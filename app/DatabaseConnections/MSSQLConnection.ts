...
/** Conexão do sqlserver (também chamado de mssql para abreviatura) utilizando a interface {@link DatabaseConnectionContract}. */
export class MSSQLConnection implements DatabaseConnectionContract {
  ...
  public stop(imediato?: boolean, callback?: (erro?: Error) => void) {
    if (!imediato)
      //se não for imediato, bota o callback antes de fechar
      this._sqlCon.on('end', () => {
        callback && callback()
      })
  ...
  private getSQLType(param: any) {
    switch (typeof param) {
      ...
      case 'object':
        if (param instanceof Date) return TYPES.DateTime
  ...
  public executeQuery(
    sql: string,
    params?: any[],
    callback?: (erro?: Error, result?: any) => void
  ) {
    ...
    let request = new Request(adaptedSql, (err, rowCount, results) => {
      callback && callback(err, this.normalizeResults(results))
      rowCount
    })
    ...
    request.on('error', (err) => {
      this.log(
        'Erro: ' +
          ((err as any).errors ?? err.message ?? objectToString(err)) +
          `\non request #${newConId}`
      )
    ...
    request.on('requestCompleted', () => {
      this.log(`Request #${newConId} complete`)
      newConnection.close() /*this._sqlCon.unprepare(request)*/
    })

    request.on('prepared', () => {
      this.log(`Prepare statement #${newConId}: ` + adaptedSql)
      newConnection.execute(request, this.paramsToObj(params, newConId))
    })
