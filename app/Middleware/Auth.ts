    ...
    if (!reqAuth) {
      this.log(
        objectToString(
          {
            path: request.url(),
            erro: 'tentativa de acesso sem o header authorization',
            host: request.host(),
      ...
      if (err) {
        this.log(
          objectToString(
            {
              path: request.url(),
              erro: 'tentativa de acesso sem token',
              description: err,
              hostname: request.hostname(),
              ip: request.ip(),
              method: request.method(),
        ...
        let msg = objectToString(
          { path: request.url(), user: (decoded as JwtPayload).userBody, dataHora },
          0
        )
        console.log(msg)
        this.log('Auth: ' + msg)
